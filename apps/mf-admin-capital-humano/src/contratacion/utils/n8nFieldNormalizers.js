/** @typedef {import('./n8nFieldNormalizers.js').*} - re-export via duplicate for Vite ESM */
...
export function parseFechaInicioSmart(value) {
    if (value == null) return null;
...
export function parseSalarioCop(value) {
    if (value == null) return null;
    if (typeof value === 'number') return Number.isFinite(value) && value > 0 ? value : null;
    const raw = String(value).trim();
    if (!raw) return null;
    if (SENTINEL_VALUES.has(stripAccentsLower(raw))) return null;
...
export function parseLegacyContactConcat(value) {
...
export function concatEmergencyContact(nombre, parentesco, telefono) {
