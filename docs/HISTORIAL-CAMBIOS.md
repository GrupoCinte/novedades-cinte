...
- **Ticket:** AUT-586
- **Requerimiento funcional:** Las horas extra rechazadas ya no reducen el cupo de recargo dominical de las aprobadas/pendientes; al rechazar o aprobar se recalcula el domingo afectado (corrige casos como 1,33/7,67).
- **Componente técnico:** `heDomingoRecargoGroup.js` (filtro estado), `registerRoutes.js` y `novedadAdminService.js` (recomputo al cambiar estado), tests, script `scripts/aut-586-remediate-domingo-rechazadas.js`.
- **Cambios (uno a uno):**
  - El grupo del tope dominical solo incluye HE Pendiente/Aprobado.
  - Al aprobar/rechazar HE (gestión y admin patch) se dispara el recomputo del intervalo.
  - Tests del caso sin rechazada (3/6/2) y del filtro SQL.
  - Script one-shot para remediar cédulas afectadas tras deploy.
