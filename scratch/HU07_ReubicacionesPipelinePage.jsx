...
function formatMontoDisplay(value, currencyCode, defaultCurrency = 'COP') {
    if (value == null || value === '') return 'ÔÇö';
...
function EstadoBadge({ estado, semaforo, isLight }) {
    ...
    return <span className="text-xs">ÔÇö</span>;
}


//COMPONENTE TIPO FICHA BADGE (igual que en Ficha Novedades)
function TipoFichaBadge({ value, isLight }) {
    const label = value || 'ÔÇö';
    
    // Si no hay valor, mostrar gui├│n sin badge
    if (!value) {
        return <span className="text-xs text-slate-400">ÔÇö</span>;
    }
...
function ReubicacionesPipelinePageInner({ token, navIntent, auth }) {
    ...
    /** Aplicar filtros enviados desde el dashboard (u otro m├│dulo) al cambiar `seq`. */
    useEffect(() => {
        ...
        <div className={dash.moduleTabShellFull}>
            ...
            >
                ...
                <div className={`flex overflow-hidden rounded-lg border ${
                    ...
                    >
                        Hist├│rico{historicoCount > 0 ? ` (${historicoCount})` : ''}
                    </button>
                </div>
                ...
                <div className={dash.tableWrap}>
                        ...
                        <table className="w-full min-w-[1100px] border-collapse text-left text-sm">
                            <thead>
                                <tr className={dash.thead}>
                                    ...
                                    <Th colKey="dias_restantes" label="D├¡as rest." align="right" />
                            ...
                            <tbody className={dash.tbody}>
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan={11} className={`p-12 text-center font-medium ${dash.muted}`}>
                                            Sin registros. Cree uno con ┬½Nuevo registro┬╗ (la c├®dula debe existir en Consultores).
                                        </td>
                                    </tr>
                                ) : (
                                    items.map((row) => (
                                        <tr key={row.id} className={dash.trHover}>
                                                ...
                                                <td
                                                    ...
                                                    onClick={() => {
                                                        if (viewMode === 'historico') {
                                                            // Ô£à Abrir modal de hist├│rico
                                                            setSelectedHistoricoRow(row);
                                                            setHistoricoModalOpen(true);
                                                        } else {
                                                            // Ô£à Abrir modal normal (con edici├│n)
                                                            setSelectedRow(row);
                                                            setModalOpen(true);
                                                        }
                                                    }}
                                            ...
                                            <td className={dash.tdCell}>{row.tipo_contrato || 'ÔÇö'}</td>
                                            <td className={dash.tdCell}>{row.cliente_actual || 'ÔÇö'}</td>
                                            <td className={dash.tdCell}>{row.cliente_destino || 'ÔÇö'}</td>
                                            <td className={dash.tdCell}>{row.puesto || 'ÔÇö'}</td>
                                            ...
                                            <td className={`${dash.tdMuted} text-right whitespace-nowrap`}>
                                                {row.dias_restantes != null ? row.dias_restantes : 'ÔÇö'}
                                            </td>
                    ...
                    {!loading && total > 0 ? (
                        <div className={dash.footerBar}>
                            <span>
                                Mostrando {rangeFrom}ÔÇô{rangeTo} de {total} ┬À P├ígina {safePage} de {totalPages}
                            </span>
            ...
            >
                <div className="flex flex-col gap-1.5">
                    ...
                    <input
                        ...
                        placeholder="C├®dula, nombre, destino o causal"
                    />
                </div>
                <div className="flex flex-col gap-1.5">
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    >
                        <option value={10}>10 por p├ígina</option>
                        <option value={20}>20 por p├ígina</option>
                        <option value={50}>50 por p├ígina</option>
                    </select>
                </div>
            </ModuleFiltersDrawer>
            ...
            <GestionModalShell
                ...
                subtitle="Crear registro de seguimiento de reubicaci├│n"
                size="md"
            >
                <form onSubmit={submitCreate} className="space-y-3 font-body">
                    <div>
                        <label className={`block text-xs ${labelMuted} mb-1`}>C├®dula *</label>
                        <input
                    ...
                    <div className="flex justify-end gap-2 pt-4 border-t border-slate-200/50 dark:border-slate-700/50">
                        ...
                        <button type="submit" disabled={createSaving} className={toolbarBtn}>
                            {createSaving ? 'GuardandoÔÇª' : 'Guardar'}
                        </button>
                    </div>
                </form>
            </GestionModalShell>
            ...
            <GestionModalShell
                ...
                subtitle={editRow ? `C├®dula ${editForm.cedula} ┬À ${editRow.consultor || 'Consultor'}` : ''}
                size="md"
            >
                <form onSubmit={submitEdit} className="space-y-3 font-body">
                    ...
                    <div className="flex justify-end gap-2 pt-4 border-t border-slate-200/50 dark:border-slate-700/50">
                        ...
                        <button type="submit" disabled={editSaving} className={toolbarBtn}>
                            {editSaving ? 'GuardandoÔÇª' : 'Guardar'}
                        </button>
                    </div>
                </form>
            </GestionModalShell>
            ...
            <GestionModalShell
                ...
                subtitle="┬┐Est├í seguro de realizar esta acci├│n?"
                size="md"
                zClass="z-[260]"
            >
                <div className="font-body space-y-4">
                    <p className={`text-sm ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>
                        ┬┐Eliminar el seguimiento de reubicaci├│n para la c├®dula{' '}
