...
async function ensureReubicacionesSchema({ pool, logger }) {
    try {
        ...
        `);
                ...
                // 8. Tabla de control de correos (HU-03)
        await pool.query(`
            CREATE TABLE IF NOT EXISTS public.alertas_correo_control (
                id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                caso_id UUID NOT NULL REFERENCES reubicaciones_pipeline(id) ON DELETE CASCADE,
                hito VARCHAR(30) NOT NULL CHECK (hito IN ('dia_0', 'dia_3', 'dia_5', 'extension', 'novedad')),
        ...
        `);
        await pool.query(`
            CREATE INDEX IF NOT EXISTS idx_alertas_control_caso 
                ON alertas_correo_control(caso_id)
        `);
        await pool.query(`
            CREATE INDEX IF NOT EXISTS idx_alertas_control_enviado 
                ON alertas_correo_control(enviado) 
                WHERE enviado = false
        `);
