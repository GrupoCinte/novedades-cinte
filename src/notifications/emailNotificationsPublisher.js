    ...
    function validateSeguimientoCierrePayload(payload) {
        ...
        if (!['consultor', 'cliente'].includes(String(payload.tipo || '').toLowerCase())) return false;
        const recipients = payload.recipients;
        if (!Array.isArray(recipients) || recipients.length === 0) return false;
        for (const r of recipients) {
    ...
    function createEmailNotificationsPublisher({
        ...
        async function publishEvent(payload, customEventType, validateFn) {
            if (!isEnabled) {
            ...
            try {
                const finalPayload = customEventType ? { ...payload, eventType: customEventType } : payload;
                const command = new InvokeCommand({
                    FunctionName: functionName,
                    InvocationType: 'Event',
                    Payload: Buffer.from(JSON.stringify(finalPayload), 'utf8')
                });
        ...
        async function publishFormSubmitted(payload) {
            return publishEvent(payload, null, validateFormSubmittedPayload);
        }
        
        async function publishFormStatusChanged(payload) {
            return publishEvent(payload, null, validateFormStatusChangedPayload);
        }
        
        async function publishConciliacionServicioFinalizada(payload) {
            return publishEvent(payload, null, validateConciliacionServicioFinalizadaPayload);
        }
        
        async function publishConciliacionCorreoLider(payload) {
            return publishEvent(payload, null, validateConciliacionCorreoLiderPayload);
        }
        
        async function publishConciliacionStakeholdersAviso(payload) {
            return publishEvent(payload, null, validateConciliacionStakeholdersAvisoPayload);
        }
        
        async function publishTimeEntryConfirmation(payload) {
            return publishEvent(payload, null, validateTimeEntryConfirmationPayload);
        }
