...
const { colaboradorVisibleEnMesSql } = require('./conciliacionTarifaProrrateo');
...
/**
 * Colaborador visible en el cierre (activo o salida en el mes), incluye inactivos.
 */
async function fetchColaboradorParaConciliacionManual(pool, ced, clienteCanon, anio, mes) {
    const visibilidadSql = colaboradorVisibleEnMesSql('c', 3, 4);
    try {
        const q = await pool.query(
            `SELECT c.cedula, c.nombre, c.correo_cinte, c.cliente, c.lider_catalogo, c.gp_user_id
             FROM colaboradores c
             WHERE regexp_replace(COALESCE(c.cedula, ''), '\\D', '', 'g') = $1
               AND lower(btrim(COALESCE(c.cliente, ''))) = lower(btrim($2::text))
               AND ${visibilidadSql}
             LIMIT 1`,
            [ced, clienteCanon, anio, mes]
        );
        return q.rows[0] || null;
    } catch (error) {
        if (String(error?.code || '') !== '42703') throw error;
        const q2 = await pool.query(
            `SELECT c.cedula, c.nombre, c.cliente
             FROM colaboradores c
             WHERE regexp_replace(COALESCE(c.cedula, ''), '\\D', '', 'g') = $1
               AND lower(btrim(COALESCE(c.cliente, ''))) = lower(btrim($2::text))
               AND ${visibilidadSql}
...
async function createConciliacionNovedadManual(deps, scope, payload, actor) {
    ...
    let colaborador = await fetchColaboradorParaConciliacionManual(pool, ced, chk.canon, y, m);
    if (!colaborador) {
        colaborador = await getColaboradorByCedula(ced);
    }
    if (!colaborador) {
...
module.exports = {
    ...
    fetchColaboradorParaConciliacionManual,
