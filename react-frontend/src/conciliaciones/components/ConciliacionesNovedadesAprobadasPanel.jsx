...
function resolveCantidadHorasNovedad(row, id, { draftCantidadHorasNovedad }) {
    ...
    return resolveCantidadHorasFacturacionPreview(row, true);
}

function enrichItemHoras(row, hoursMode) {
    if (!hoursMode || !isNovedadCalculadaHoras(row)) return row;
    const horas = resolveCantidadHorasFacturacionPreview(row, true);
...
export default function ConciliacionesNovedadesAprobadasPanel({
    ...
    const displayItems = useMemo(() => {
        ...
        if (!editMode) {
            ...
            return baseList.map((row) => enrichItemHoras(row, true));
        }
