...
export const DEFAULT_HORA_INICIO = '22:00';
export const DEFAULT_HORA_FIN = '06:00';

/** Ventana nocturna única: 18:00–23:59 y 00:00–06:00 (24 h). */
export const NOCTURNO_EVENING_MIN = 18 * 60;
export const NOCTURNO_MORNING_MAX = 6 * 60;
export const NOCTURNO_MAX_HORAS = 12;
...
export function isNocturnoTime(minutes) {
...
export function formatFranjaLabel(horaInicio, horaFin, cantidadHoras) {
    const h =
        cantidadHoras % 1 === 0 ? String(Math.round(cantidadHoras)) : String(cantidadHoras);
    return `${horaInicio}–${horaFin} (${h} h)`;
}

export function validateShiftTimes(horaInicio, horaFin) {
    const start = parseHhMm(horaInicio);
    const end = parseHhMm(horaFin);
