...
/** El Empleo: mismo flujo que LinkedIn (extensión). Playwright headed solo como fallback. */
function ElEmpleoCard({
    row,
    token,
    isLight,
    onRefresh,
    ...
    const waitingWorker = workerPending || row.estado === 'conectando';
    const isRenewing = loginOpened && (row.estado === 'conectado' || row.estado === 'expirado');
    const isConnecting = (!waitingWorker && (row.estado === 'conectando' || (loginOpened && row.estado !== 'conectado' && row.estado !== 'expirado')));
    const needsConnect = row.estado !== 'conectado' && row.estado !== 'expirado' && !isConnecting && !isRenewing && !waitingWorker;
    const showSaveSession = (isConnecting && row.estado !== 'conectado') || isRenewing;
    const badgeEstado = waitingWorker || isRenewing || isConnecting ? 'conectando' : row.estado;

    async function onConnect() {
        setError('');
        setBusy(true);
        try {
            await connectIntegracion(token, 'elempleo');
            openProviderWorkspaceTab('elempleo');
            setLoginOpened(true);
            onRefresh({ poll: true });
        } catch (e) {
    ...
    async function onSaveSession(ev) {
        ...
        setBusy(true);
        try {
            await captureSessionViaExtension('elempleo', token);
    ...
    async function onConnectWorkerFallback() {
        setError('');
        setBusy(true);
        ...
        try {
            ...
            setError(e.message || 'No se pudo abrir el navegador automático.');
        } finally {
            setBusy(false);
        }
    }
    ...
    }, [waitingWorker, onRefresh]);

    useEffect(() => {
        if (!workerPending || row.estado !== 'conectado') return undefined;
        setWorkerPending(false);
        onSessionSaved?.('elempleo');
        return undefined;
    }, [workerPending, row.estado, onSessionSaved]);
        ...
        <li className={`rounded-xl border px-5 py-4 ${cardInner}`}>
            ...
            {showSaveSession ? (
                <ol className={`mt-3 list-decimal space-y-1 pl-4 text-xs ${muted}`}>
                    <li>Inicie sesión en El Empleo con la cuenta <strong>empresa</strong> (buscador de candidatos).</li>
                    <li>Vuelva aquí y pulse <strong>Guardar conexión</strong>.</li>
                </ol>
            ) : (
                <p className={`mt-3 text-sm ${muted}`}>
                    Instale el conector CINTE, abra El Empleo, inicie sesión como empresa y guarde la conexión aquí.
                </p>
            )}

            {waitingWorker ? (
                <p className="mt-3 text-sm text-amber-800">
                    Navegador automático abierto. Complete el login y no cierre la ventana hasta ver «Conectado».
                </p>
            ) : null}
            ...
            {extensionStale ? (
                ...
                <p className="mt-3 text-xs text-amber-700">
                    Instale el conector CINTE (carpeta{' '}
                    <code className="rounded bg-amber-100 px-1">{EXTENSION_PATH}</code>
                    ): Chrome → Extensiones → Modo desarrollador → Cargar descomprimida.
                </p>
            ) : (
                <p className={`mt-3 text-xs ${isLight ? 'text-emerald-700' : 'text-emerald-300'}`}>Conector detectado.</p>
            )}

            {row.mensaje && row.estado === 'conectado' && !isRenewing && !waitingWorker ? (
                <p className={`mt-2 text-xs ${isLight ? 'text-emerald-700' : 'text-emerald-300'}`}>{row.mensaje}</p>
            ) : null}
            ...
            <div className="mt-4 flex flex-wrap gap-2">
                ...
                {showSaveSession ? (
                    <button type="button" className={`${CINTE_BTN_PRIMARY} !px-4 !py-2 !text-xs`} disabled={busy || !extensionReady} onClick={onSaveSession}>
                        {busy ? 'Guardando…' : '2. Guardar conexión'}
                    </button>
                ) : null}
                {(row.estado === 'conectado' || row.estado === 'expirado') && !isRenewing && !waitingWorker ? (
                ...
                {!waitingWorker ? (
                    <button type="button" className={btnSecondary} disabled={busy} onClick={onConnectWorkerFallback}>
                        Si falla la extensión: navegador automático
                    </button>
                ) : null}
            </div>
