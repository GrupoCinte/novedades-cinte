    ...
    return (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
            <div
                ...
                onClick={onClose}
            ></div>
            <div className="modal-glass-sheet relative flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-[var(--border)] p-0 shadow-2xl font-body">

                <div className="flex items-center justify-between border-b border-[var(--border)] bg-[var(--surface-soft)] px-6 py-5">
                    <div>
                        <h2 className="text-2xl font-bold text-[var(--text)] tracking-tight font-heading">{selectedUser.workflowName}</h2>
                        <p className="mt-0.5 text-sm font-mono text-[var(--muted)]">{selectedUser.executionId}</p>
                    </div>
                    <button
                        onClick={onClose}
                        className="rounded-lg p-2 text-[var(--muted)] transition-all hover:bg-[var(--surface-muted)] hover:text-[var(--text)]"
                    >
                ...
                <div className="flex items-center justify-between border-b border-[var(--border)] bg-[var(--surface-soft)] px-6 py-3">
                    <div className="flex items-center gap-3">
                        ...
                        <span className={`text-sm font-medium uppercase tracking-wide ${
                            selectedUser.realStatus === 'analizando' ? 'text-cinte-warning' : 'text-[var(--muted)]'
                        }`}>
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar">
                    {isEliminado && obsElim ? (
                        <div className="mb-5 rounded-xl border border-red-500/40 bg-transparent px-4 py-3">
                            <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--error)]">Motivo de eliminación</p>
                            <p className="mt-2 whitespace-pre-wrap text-sm leading-relaxed text-[var(--text)]">{obsElim}</p>
                        </div>
                    ) : null}
                        ...
                        {selectedUser.fullData ? (
                            detailEntries.map(([key, value]) => (
                                <div key={key} className="rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-4 transition-colors hover:border-[var(--primary)]/40">
                                    ...
                                    <div className="break-words text-sm font-medium leading-relaxed text-[var(--text)]">
                                        {formatDetailDisplay(key, value)}
                                    </div>
                                </div>
                            ))
                        ) : (
                            <div className="col-span-2 text-center py-8 italic text-[var(--muted)]">
