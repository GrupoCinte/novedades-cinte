...
import { NAV_COLABORADOR } from '../navigation';
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-[calc(100vh-4rem)]">
      <aside className="w-full md:w-64 shrink-0 border-b md:border-b-0 md:border-r border-slate-800 bg-[#0f172a] p-4">
        <p className="text-xs font-bold uppercase tracking-wider text-cyan-400/90 mb-3">Portal colaborador</p>
        <p className="text-xs text-slate-500 mb-4">Cliente interno</p>
        <nav className="flex flex-row md:flex-col gap-1 overflow-x-auto md:overflow-visible pb-2 md:pb-0">
          {NAV_COLABORADOR.map((item) => (
            <NavLink
              key={item.id}
              to={item.id}
              className={({ isActive }) =>
