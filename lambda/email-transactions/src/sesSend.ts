...
import { SESClient, SendRawEmailCommand, type SendRawEmailCommandOutput } from '@aws-sdk/client-ses';
 ...
 */
export async function sendHtmlEmailWithInlineLogo(
  sesClient: SESClient,
  ...
  const toList = (Array.isArray(opts.to) ? opts.to : [opts.to])
    .map((e) => String(e || '').trim())
    .filter((e) => e.includes('@'));
  if (!toList.length) throw new Error('destinatario inválido');
